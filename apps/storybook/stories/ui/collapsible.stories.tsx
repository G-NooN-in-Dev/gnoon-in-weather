...
const meta = {
	...
	render: ({ open, heading, content }) => (
		<Collapsible open={open} className="w-[350px] space-y-2">
			<div className="flex items-center justify-between gap-4">
				<h4 className="text-sm font-medium">{heading}</h4>
				<CollapsibleTrigger render={<Button variant="ghost" size="sm" />}>{open ? '닫기' : '열기'}</CollapsibleTrigger>
			</div>
